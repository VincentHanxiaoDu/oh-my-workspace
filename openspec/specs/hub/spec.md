...
**One process per company that holds the published corpus and answers questions about it** (PRD
§2.2). Notes live somewhere that outlives the person who wrote them and the machine they wrote them
on, so the hub's first duty is durability: a note published into a hub directory is readable, under
the identifier it was given, by a process started against that directory later.

Two things follow from that duty and are the substance of this capability. A hub that cannot read
or cannot write its own record **determines nothing rather than reporting nothing** (PRD §4.3) — an
unparseable record, an unfinished write, or a full disk produces an undetermined answer or a halt,
never an empty corpus that reads as "nothing was written". And every answer is settled against who
is asking before anything is ranked, so an unidentified caller gets no determined answer and a note
narrowed away from a reader is absent from their results rather than present and filtered late.

The hub is not a wall against whoever operates it. It holds every published note in full, including
notes narrowed to named people, and it says so in the product's own words rather than leaving an
operator to infer it (PRD §4.5).

**This capability is the company half, and the local half does not depend on it** (PRD §4.4). Every
local capability works with no hub configured; a hub existing does not make the client require one.
