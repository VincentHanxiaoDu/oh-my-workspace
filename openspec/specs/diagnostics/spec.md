...
## Purpose
Something is wrong with a person's client and they need help from whoever supports them. But
everything on this machine is their material — their tickets, their unpublished drafts, the messages
that came in from their channels. They want to hand over enough for someone to diagnose the problem,
and they want to know, before they press send, exactly what they are handing over and what they are
not. Diagnostics exists for that one moment.

A bundle therefore says what it contains. The manifest travels inside the bundle, so the person
holding it — and the person receiving it — can read what is in it without opening a single byte of
the collected data (§3.9). Every category the bundle could hold has a line in that manifest, whether
or not anything was collected for it. Nothing is silently dropped: a bundle that quietly leaves
things out is worse than one that says plainly "I left N ticket bodies out", because the second one
can be trusted and the first one cannot.

By default it withholds the person's material. Ticket bodies, draft note bodies and raw message
bodies ingested from any channel are not in a default bundle — the containers §2.3 says never leave
the machine except by publication do not leave it through the support channel either. Bodies are
included only when the person affirmatively asks for them, never as a default and never implied by
some other option, and when they are included the manifest says so in the same field that otherwise
reads "withheld". There is no bundle whose manifest understates what it holds. The model key is not
in the switch at all: it is never collected, under any option.

Undetermined is an answer here, not an absence (§4.3). Each category is collected, withheld, or
undetermined, and undetermined carries its reason — withheld by default, could not be read, no store
on this machine, no hub configured. A category unavailable because there is no store reads differently
from one that was collected and found empty, and neither reads as a negative finding. A hub-derived
category with no hub configured is named as such, never rendered as bad news and never dropped.

It works when the product does not. Health reporting needs neither a store nor a running daemon
(§4.1), and the daemon being dead is the usual reason to need a bundle in the first place, so a
bundle is produced with no daemon and with no store at all — and those two situations, and a healthy
one, are distinguishable from the manifest alone. Producing a bundle starts no daemon, and its
account of daemon state is the product's single liveness answer, not a second opinion. The health
facts of §4.1 and §5.1 ride along: full-disk encryption in its three values, and whether owner-only
permissions on the control API could be confirmed.

Finally, it refuses to act on the person's behalf. Producing a bundle opens no network connection
when no hub is configured and transmits the bundle nowhere, ever — handing it over is the person's
act and only theirs. And a bundle that exists is a complete one: on failure the command exits
non-zero and leaves nothing behind that could be mistaken for a finished artifact.
## Requirements
